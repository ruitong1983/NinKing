...
| 卡牌背面 | 1 | 程序绘制 140×196, 像素风, PNG | **P0** | ✅ `CardBackGenerator` (手里剑十字星+交叉纹底+金边框) |
...
**像素风格化进度（V16-V20 已实施）：**
- ✅ 全局像素风 Theme (`pixel_theme.tres`): 0圆角/2px硬边/像素三态按钮
- ✅ 8 結界冷暖交替配色 (`barrier_theme.gd`)
- ✅ CRT 扫描线 + 微下移动效
- ✅ 卡牌背面程序绘制 (`card_back.png`)
- ✅ 忍者槽位背景程序绘制 (`slot_bg.png`)
- ⬜ 花色 ♠ ♥ ♦ ♣ 像素化绘制
- ⬜ 人牌 (J/Q/K): 简化的像素人物头像
...
| 忍者牌槽位背景 | 100×140, 像素风边框, PNG | **P0** | ✅ `CardBackGenerator.generate_slot_bg()` (深蓝底+金边框+内凹区+手里剑) |
| 按钮背景 (普通) | 可拉伸, 像素风, PNG | P1 | ✅ 由 V16 `pixel_theme.tres` StyleBoxFlat 程序化覆盖 |
| 按钮背景 (按下) | 可拉伸, 像素风, PNG | P1 | ✅ 同上 |
...
| 文件 | 用途 | 优先级 |
|---|---|---|
| `assets/themes/pixel_theme.tres` | 全局像素风主题（字体/颜色/按钮样式） | **P0** | ✅ V16 重写：0圆角/2px硬边/三态按钮/三墩边框递进 |
| `scripts/ninking/barrier_theme.gd` | 8 結界冷暖交替配色表 | P1 | ✅ V17 新建：每結界 bg/panel/accent/name |

### 主题关键配置（V16 更新）

- Default Font: Press Start 2P + 凤凰点阵体 12px/16px (CJK 回退)
- Font Size: 主 UI 16~24px, 标题 32px
- Button StyleBox: 2px 硬边框, 0 圆角, content_margin 下移按压态
- Color Palette: 动态 8 結界冷暖交替 (紫/红/青/橙/蓝/金/翠/粉) via `BarrierTheme`
...
### Phase 1 — 可玩 (P0) ✅ 全部完成
52 张卡牌 (✅) + 卡牌背面 (✅ V1) + icon (✅) + 像素字体 (✅ V2) + pixel_theme.tres (✅ V16) + 槽位背景 (✅ V6)

### Phase 2 — 完整体验 (P1)
商店面板 + VFX 接入 (V10-V12 ✅) + CRT (✅ V18) + 結界配色 (✅ V17)
- 音频: 已有占位 (来自 FanKing)，后续替换为扑克风格

### Phase 3 — 润色 (P2-P3)
商店 BGM + Boss 墨字 (✅ V14) + 屏风转场增强 (V15)
