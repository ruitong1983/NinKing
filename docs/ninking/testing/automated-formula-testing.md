...
在 Godot 中加载 `debug_ninking_main.tscn`，手动输入测试牌组，对比 CSV 预测值。
...
| 列牌型导致高基线 | 列独立计分抬高基线 | 区分"纯效果"和"列分复合效果" |

### 3.3 标准"安全牌组" (SD)

```python
# 无喜干扰的标准牌组（v5.0 基线 313）
SD = ['♠2','♥7','♦J'], ['♣4','♠9','♥Q'], ['♣3','♠6','♥K']
# 特点：全散牌，列0顺(chips=87)，列1散(0)，列2顺(chips=150) → 基线 313
# 适用于：成长卡/通用卡的基线对比
```
...
```python
# 三列豹牌组（高列分验证 — v5.0 列独立计分）
['♠3','♥7','♦J'], ['♦3','♠7','♥J'], ['♣3','♣7','♠J']
# 3个点数各3张 → 三列都豹(~1000分/列) + 全三条×4
...
def main():
    ...
    # 4. 纯效果分析
    print('\n--- 4. 纯效果(无列/无喜) ---')
    pure = [p for p in pairs if p['n']['col_total'] == '0'
...
3. 列分+喜乘复合导致高 delta → 属正常放大，检查公式即可
```
