...
当前 `owned_ninjas: Array[Dictionary]` 每次出牌被完整遍历 **至少 7 次**，每次做同样的 `.get("effect", {})` 字典解析，且解析逻辑散布在 5 个文件中。同时 4 种工具类效果（extra_plays/extra_redraws/death_save/wild）**悬空未处理**。
...
### Phase 1e — 工具效果可见性
- `analyze_effects()` 中收集 extra_plays/extra_redraws/death_save 到 `summary.tools`
