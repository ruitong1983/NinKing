...
```
 闪屏（0.8s）
    │
    ▼
 主菜单（4 按钮 stagger slide-in）
   ├── [開始] → 牌组选择面板 → [确认] → change_scene → Main
   ├── [繼續] → 继续确认面板 → [继续冒险] → change_scene → Main
   ├── [設置] → (预留)
   └── [退出] → quit()
```
...
| 按钮 | text | 默认状态 | 备 注 |
|------|------|---------|------|
| StartBtn | `開始` | 可用 | 打开牌组选择面板 |
| ContinueBtn | `繼續` | 无存档时 disabled | 打开继续确认面板 |
| SettingsBtn | `設置` | 可用 | 预留，暂无功能 |
| QuitBtn | `退出` | 可用 | `get_tree().quit()` |
...
[開始] click
  ...
  ├── [確認] → start_new_run() + change_scene → Main
...
Launch                            Main
  │                                 │
  ├─ start_new_run(deck_key) ──────→ NinKingGameState 初始化
  │  change_scene_to_file()          game_manager._ready()
  │                                  ├── 加载场景
  │                                  ├── 发牌
  │                                  └── SEAL_INTRO → 2s → PLAYING
  │
