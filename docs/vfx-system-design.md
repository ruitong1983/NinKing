...
> 设计日期：2026-06-06
> 实现日期：2026-06-06
> 最后更新：2026-06-23 (+ GlobalShaders Shader 子系统：色差/故障/像素爆炸/滤镜)
> 状态：✅ 框架已实现 + 上层对接完成
...
scripts/shader/                           # Shader 特效子系统（全部 class_name，可移植）
├── global_shaders.gd    (~230 行) ❌ 不可移植：本项目 Autoload 胶水层
├── shader_fx.gd         (~150 行) ✅ 纯静态 Shader 工具 (Autoload: ShaderFX)
├── edge_fade_fx.gd      ( 60 行) ✅ 面板边缘淡出 (shaders/panel_edge_fade.gdshader)
├── dissolve_fx.gd       (~140 行) ✅ 噪声溶解消散 (shaders/fake3d/dissolve2d.gdshader)
├── glow_fx.gd           (~120 行) ✅ Sprite 内发光 (shaders/baked_sprite_glow.gdshader)
├── outline_fx.gd        ( 60 行) ✅ 内外描边 (shaders/outline2D_inner_outer.gdshader)
├── chromatic_aberration_fx.gd ( 80 行) ✅ 色差故障 (shaders/effects/chromatic_aberration.gdshader)
├── split_glitch_fx.gd   (~100 行) ✅ 分裂故障扰动 (shaders/effects/split_glitch.gdshader)
└── pixel_explosion_fx.gd(~120 行) ✅ 像素爆炸消散 (shaders/effects/pixel_explosion.gdshader)

shaders/
├── panel_edge_fade.gdshader  ( 30 行) ✅ 面板边缘渐隐着色器
├── baked_sprite_glow.gdshader(110 行) ✅ Sprite 内发光 (gdquest)
├── outline2D_inner_outer.gdshader ( 80 行) ✅ 内外描边 (gdquest)
├── filters/
│   └── multi_filter.gdshader  ( 70 行) ✅ 复合滤镜（灰度/暗角/颜色叠加/混合）
├── effects/
│   ├── chromatic_aberration.gdshader ( 75 行) ✅ 色差
│   ├── split_glitch.gdshader         ( 67 行) ✅ 分裂故障扰动
│   └── pixel_explosion.gdshader      ( 45 行) ✅ 像素爆炸消散
...
├── shaderlib/                        # ⛔ 已清空 (2026-06-23): 3 未被覆盖 + 2 已覆盖 全部删除
└── sources/                          # 外部来源存档，见 SOURCES.md
...
**总计：~1447 行 tween 脚本 + ~820 行 shader 脚本 + ~530 行 shader 源码**
...
┌──────────────────────────────────────────────────────────┐
│                    调用方（项目各处）                       │
│       卡牌类 / 结算类 / UI按钮 / 场景切换 / VFX              │
├──────────────────────────┬───────────────────────────────┤
│  GlobalTweens (Tween 层)  │  GlobalShaders (Shader 层)    │
├──────────┬──────────┬────┼──────┬──────────┬────────────┤
│ TweenFX  │Shake/Hit │Crd │SFX   │EdgeFade  │DissolveFX  │
│ (Autoload│ Stop/Tilt│Tilt│(工具)│FX        │            │
│  纯静态) │ (实例)   │    │      │          │            │
├──────────┼──────────┼────┼──────┼──────────┼────────────┤
│ CountUp  │ Particle │Audio│GlowFX│OutlineFX │Chromatic   │
│          │ Pool     │Cplr│      │          │AbernFX     │
├──────────┴──────────┴────┼──────┴──────────┼────────────┤
│  shaders/fake3d/         │SplitGlitchFX    │PixelExplode│
│  fake3d/flash/dissolve2d │                 │FX          │
├──────────────────────────┴───────────────────────────────┤
│  shaders/ (panel_edge_fade / baked_sprite_glow / outline) │
│  shaders/filters/multi_filter / shaders/effects/          │
└──────────────────────────────────────────────────────────┘
```

Tween 层与 Shader 层通过 `ShaderFX.tween_param()` / `ShaderFX.pulse_param()` 互通——Shader 层委托 GlobalTweens 做参数补间。外部代码统一走 `GlobalTweens.xxx()` 或 `GlobalShaders.xxx()`，不直接调子系统。
...
GlobalShaders="*res://scripts/shader/global_shaders.gd"
ShaderFX="*res://scripts/shader/shader_fx.gd"
```
...
### 10. GlobalShaders（Shader 特效胶水层）

**文件**：`scripts/shader/global_shaders.gd` · Autoload 名 `GlobalShaders` · ~230 行
**不可移植**：本项目专属胶水层
**依赖**：`ShaderFX`（纯函数库）+ 7 个子系统

#### 核心 API（调用方只接触 GlobalShaders）

| 分类 | API | 效果 |
|------|-----|------|
| 边缘淡出 | `apply_edge_fade(node)` / `clear_edge_fade(node)` | 面板水墨边缘 |
| 溶解消散 | `apply_dissolve(node, params)` / `dissolve_out(node, params)` → Tween | 噪声溶解 + 燃烧边框 |
| 辉光 | `apply_glow(node, params)` / `pulse_glow(node, params)` → Tween | Sprite 内发光 + 呼吸脉冲 |
| 描边 | `apply_outline(node, params)` / `clear_outline(node)` | 内外描边（卡牌选中高亮） |
| 色差 | `apply_chromatic_aberration(node, params)` / `clear_chromatic_aberration(node)` | RGB 偏移故障/受击反馈 |
| 分裂故障 | `apply_split_glitch(node, params)` / `split_glitch_burst(node, params)` → Tween | 网格扰动 + 爆发动画 |
| 像素爆炸 | `apply_pixel_explosion(node, params)` / `pixel_explode(node, params)` → Tween | 像素消散动画 |
| 参数动效 | `tween_param(mat, param, to, dur)` / `pulse_param(mat, param, min, max, cycle)` | Shader 参数补间/呼吸 |
| 批量清理 | `remove_all_shaders(node)` | 一键移除节点上所有 shader 材质 |

#### 设计要点

- `ShaderFX` 是纯函数库，提供 `load_shader()` / `create_material()` / `apply_material()` 等基础工具
- 每个 `*_fx.gd` 子系统是一个 `class_name Node`，管理单一 shader 的生命周期（apply / cleanup / is_applied）
- 子系统通过 `_active: Dictionary` 追踪已应用的节点→材质映射，防止内存泄漏
- 与 Tween 层互通：`ShaderFX.tween_param()` / `pulse_param()` 委托 `GlobalTweens` 做参数补间
- 同一节点不能同时挂多个 ShaderMaterial（后挂覆盖先挂），需要叠加时合并到一个 shader 中

详细参数说明 → `docs/shader-library-reference.md`
...
```
GlobalTweens (Tween 层) ──┬── TweenFX         (Autoload, 纯静态) — 含 move_arc / dissolve_out
...
GlobalShaders (Shader 层) ─┬── ShaderFX      (Autoload, 纯静态) — 工具函数
                           ├── EdgeFadeFX     (shaders/panel_edge_fade.gdshader)
                           ├── DissolveFX     (shaders/fake3d/dissolve2d.gdshader)
                           ├── GlowFX         (shaders/baked_sprite_glow.gdshader)
                           ├── OutlineFX      (shaders/outline2D_inner_outer.gdshader)
                           ├── ChromaticAberrationFX (shaders/effects/chromatic_aberration.gdshader)
                           ├── SplitGlitchFX  (shaders/effects/split_glitch.gdshader)
                           └── PixelExplosionFX (shaders/effects/pixel_explosion.gdshader)

Tween 层 ↔ Shader 层: ShaderFX.tween_param/pulse_param → 委托 GlobalTweens 做参数补间
```
...
| **ChromaticAberrationFX** (色差) | ✅ | 2026-06-23 | `shaders/effects/chromatic_aberration.gdshader` + `chromatic_aberration_fx.gd` |
| **SplitGlitchFX** (分裂故障) | ✅ | 2026-06-23 | `shaders/effects/split_glitch.gdshader` + `split_glitch_fx.gd` + burst 动画 |
| **PixelExplosionFX** (像素爆炸) | ✅ | 2026-06-23 | `shaders/effects/pixel_explosion.gdshader` + `pixel_explosion_fx.gd` + explode 动画 |
| **MultiFilter** (复合滤镜) | ✅ | 2026-06-23 | `shaders/filters/multi_filter.gdshader` — 灰度/暗角/颜色叠加/混合 |
| **GlobalShaders 注册 + 文档** | ✅ | 2026-06-23 | 统一入口集成 3 新子系统 + `shader-library-reference.md` 同步 |
