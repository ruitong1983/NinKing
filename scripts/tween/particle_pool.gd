...
func _init() -> void:
	_placeholder_tex = _make_placeholder_texture()
...
func burst(position: Vector2, preset: String = "sparkle") -> void:
	...
	_burst_internal(position, cfg.amount, cfg.lifetime, cfg.color, cfg.spread, cfg.velocity_range)
...
class PresetConfig:
	...
	func _init(p_amount: int, p_lifetime: float, p_color: Color, p_spread: float, p_vel_range: Vector2) -> void:
		amount = p_amount
		lifetime = p_lifetime
		color = p_color
		spread = p_spread
